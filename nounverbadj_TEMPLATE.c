...
// Adjective list: common adjectives for demonstration
const char *adjective_list[] = {
    "nice", "fast", "beautiful", "smart", "tall", "smelly", "young", "old", "green", "blue"
};
const int adjective_list_size = sizeof(adjective_list) / sizeof(adjective_list[0]);
...
// Function to check if a word is an adjective (classification: adjective detector)
int check_if_adjective(const char *word) {
    for (int i = 0; i < adjective_list_size; i++) {
        if (strcmp(word, adjective_list[i]) == 0) {
            return 1; // is adjective
        }
    }
    return 0; // not adjective
}
